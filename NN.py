...
class MyNN():
    ...
    def forward_pass(self, inputs):
        ...
        """
        activations = [inputs] # arr(1,0)(1,2) * w(2,3)
    ...
    def backpropagate(self, inputs, true_output):
        ...
        Args:
        inputs (ndarray): The input data.
        true_output (ndarray): The expected output.
        ...
        grad_output = error * self.sigmoid_derivative(activations[-1])
        grads_w = []
        grads_b = []

        # Backpropagate the gradients for each layer
        for i in reversed(range(len(self.weights))):  # n-1, n-2, ... 3, 2, 1, 0
            ...
            if i > 0:
                grad_output = np.dot(grad_output, self.weights[i].T) * self.sigmoid_derivative(activations[i])
        grads_w = grads_w[::-1]
        grads_b = grads_b[::-1]
